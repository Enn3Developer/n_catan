...
func show_leave(net: CatanNetwork,finished: bool):
	var hosting=net.multiplayer.is_server() and not net.dedicated
	if finished:%Message.text=tr("This game is over. You go back to the main menu.")
	elif net.tutorial:%Message.text=tr("The lesson ends here. You can start it again from the main menu.")
