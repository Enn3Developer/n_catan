...
func label(parent: Node,text: String,size: int,color: Color=Color("f4e8ce")) -> Label:
	var n=Label.new();n.text=text;n.add_theme_font_size_override("font_size",size);n.add_theme_color_override("font_color",color);parent.add_child(n);return n
func button(parent: Node,text: String) -> Button:
	var n=Button.new();n.text=text;n.custom_minimum_size.y=42;parent.add_child(n);return n
...
func setup(settings: CatanSettings,net: CatanNetwork):
	...
	var title=label(heading,"Piece cosmetics",28);title.size_flags_horizontal=Control.SIZE_EXPAND_FILL
	...
	label(preview_box,"Road · Settlement · City",14,Color("dcb978")).horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER
	label(preview_box,"Drag the preview to turn the pieces",14,Color("9ab4be")).horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER
	...
	equip=button(bottom,"Equip set");equip.name="EquipSet";equip.custom_minimum_size.x=130;equip.pressed.connect(_equip)
...
func refresh_roster():
	...
	if network.is_controller():
		for i in network.roster.size():
			if network.roster[i].get("bot",false):
				choices.add_item(network.roster[i].name+" · Bot")
	...
	updating=false
	if is_instance_valid(equip):_status()

func _target_changed(index: int):
	if updating:return
	target=choices.get_item_metadata(index);preview_color=CatanBoard.PLAYERS[maxi(0,target)]
	select_style(_equipped())
...
func _status():
	var equipped=_equipped()==selected
	equip.disabled=equipped
	equip.text="Equipped" if equipped else "Equip set"
	status.text=CatanCosmetics.SETS[selected]+(" · Equipped" if equipped else " · Preview")
