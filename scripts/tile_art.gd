...
# Sea rocks stand in full sun on open water, so they need a darker, warmer stone than the tiles.
const SURFACES={"PBR_Rock":Color("83939d"),"PBR_SeaRock":Color("675f56"),"PBR_Wood":Color("b39162"),"PBR_Bark":Color("74543c"),"PBR_Clay":Color("bd7655"),"Brick":Color("ba7555"),"Sandstone":Color("d0ac79"),"Plaster":Color("dfcba3"),"Roof":Color("657a85")}
...
func surface(name: String,base: Material,values: Dictionary) -> Material:
	...
	if LEAVES.has(name):
		...
		materials[name]=leaf
		return leaf
	var sources={"PBR_Rock":"rock_boulder_dry","PBR_SeaRock":"rock_boulder_dry","PBR_Wood":"weathered_brown_planks","PBR_Bark":"pine_bark","PBR_Clay":"brown_mud_dry","Brick":"brown_mud_dry","Sandstone":"rock_ground","Plaster":"rock_ground","Roof":"weathered_brown_planks"}
	if sources.has(name):
		...
		surface_material.set_shader_parameter("texture_strength",.32 if name=="PBR_SeaRock" else .10)
