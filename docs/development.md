...
`tools/build_seafaring.py` builds its models from code with Blender's Python module: run `python3 tools/build_seafaring.py` with the `bpy` package installed, or `blender --background --python tools/build_seafaring.py`. It saves the `.blend` and exports each glTF itself. Keep the hull's faces pointing outward; the script asserts it, because the textured wood shader culls back faces.
