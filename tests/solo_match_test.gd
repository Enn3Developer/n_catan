...
func run():
	...
	for difficulty in 3:net.configure_bot("add",-1,difficulty)
	net.start_game()
	net.bot_delay=0.09
	...
	print("SOLO_MATCH_TEST: winner=",latest.winner," ticks=",iterations," rejections=",notices.size()," passed=",passed)
	net.leave()
