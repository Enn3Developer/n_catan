...
func run():
	...
	check(game.modal.tracks.size()==5,"library lists all five tracks")
	game.modal.tracks[2].pressed.emit();await settle()
	check(game.net.music_state().track==2 and "Trade Winds" in game.modal.now_playing.text,"playlist selection changes track and label")
	game.modal.toggle.pressed.emit();await settle()
	...
	check("Sunlit Fields" in game.modal.now_playing.text,"music screen refreshes across track changes")
	game._set_music_volume(.23);await settle()
	check(is_equal_approx(game.modal.volume.value,.23),"music screen reflects personal volume")
