...
func run():
	...
	check(game._node("NowPlaying")!=null and game._node("MusicLibraryAction")!=null,"persistent in-game music section")
	game._open_music();game.net._sync();await settle()
	...
	check("Sunlit Fields" in game._node("NowPlaying").text,"persistent music row refreshes without rebuilding HUD")
	game._set_music_volume(.23);await settle()
	check(is_equal_approx(game.music_widgets.volume.value,.23) and is_equal_approx(game.music_dialog_widgets.volume.value,.23),"personal volume controls stay consistent")
	for dimensions in [Vector2i(800,600),Vector2i(1440,900)]:
		...
		check(bounds.encloses(game._node("MusicBody").get_global_rect()),"music section fits "+str(dimensions))
