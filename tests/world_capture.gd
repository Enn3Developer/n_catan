...
func run():
	...
	board.camera_focus=Vector3(0,1.9,0);board.camera_zoom=.85;board.pitch=.85;board.yaw=.2;board._update_camera()
	await shot("overview")
	for kind in 6:
		for tile in data.tiles:
			if tile.kind!=kind:continue
			board.camera_focus=Vector3(tile.x*10,2.7,tile.z*10)
