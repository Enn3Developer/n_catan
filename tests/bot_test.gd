...
func run():
	for game in 12:
		...
		rules.create(["One","Two","Three","Four"],711+game*191)
		var brains=[]
		for p in 4: brains.append(CatanBot.new(game*121+p+1))
		var steps=0
		var turns=0
		while rules.s.winner==-1 and steps<6000:
			...
			var difficulty=game/3 if game<9 else p%3
			...
			for res in 5:
				...
				if total!=19: failures+=1; printerr("Resource imbalance")
		if rules.s.winner<0:
			failures+=1
		...
		actions+=steps
	print("BOT_TEST: 12 full games, ",actions," actions, ",failures," failures")
