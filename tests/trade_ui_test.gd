...
func run():
	...
	check(game.modal.selected_give.get_item_icon(0)!=null,"trade selectors show resource icons")
	check(button("Players").button_pressed and button("Offer to all players").visible,"player trading opens by default")
	check(game.modal.amount_give.value==1 and game.modal.amount_get.value==1,"player offer defaults to 1:1")
	...
	check(game.modal.amount_give.value==4 and game.modal.amount_get.value==1,"bank shows 4:1 quantities")
	check(not game.modal.amount_give.editable and not button("Bank trade").disabled,"bank amount fixed and affordable")
	game.modal.selected_get.select(0);game.modal.selected_get.item_selected.emit(0)
	check(button("Bank trade").disabled,"same-resource trade unavailable")
	game.modal.selected_get.select(1);game.modal.selected_get.item_selected.emit(1)
	button("Players").button_pressed=true;button("Players").pressed.emit()
	check(game.modal.amount_give.editable and button("Offer to all players").visible,"player offer allows custom amount")
	check(game.modal.amount_give.value==1 and game.modal.amount_get.value==1,"switching from bank returns to 1:1 player offer")
	game.modal.amount_give.value=5
	...
	check(button("Players").button_pressed and game.modal.amount_give.value==1,"reopening starts a new player offer")
	button("Bank").button_pressed=true;button("Bank").pressed.emit()
	check(game.modal.amount_give.value==2 and game.modal.amount_get.value==1,"owned resource port changes exchange quantity")
	...
	check(button("Bank").button_pressed and not game.modal.amount_give.editable,"paired turns default to bank instead")
