...
func run():
	...
	check(game.board.pieces_root.find_children("Settlement_*","Node3D",true,false).size()==3,"settlements rendered")
	check(game.board.pieces_root.find_children("City_*","Node3D",true,false).size()==3,"cities rendered")
	...
	check(game.board.pieces_root.find_children("City_Voyager","Node3D",true,false).size()==1,"equipped city rebuilt in game")
