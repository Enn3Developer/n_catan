...
func run():
	...
	board.day_seconds=150;board.advance_day(0)
	check(farmer.root.visible and is_equal_approx(sheep.root.scale.y,1),"dawn restores work and grazing")
	...
	if DisplayServer.get_name()!="headless":
		...
		board.view_region=Rect2();board.camera_focus=Vector3(vertex.x*10,2.4,vertex.z*10);board.camera_zoom=.19;board._update_camera()
		for time in [150,450]:
		...
		board.camera_focus=Vector3(tile.x*10,2.4,(tile.z+.82)*10);board.camera_zoom=.14;board._update_camera()
		await create_timer(.7).timeout;await RenderingServer.frame_post_draw
