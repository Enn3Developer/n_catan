...
The host sets the room rules in the lobby: the map seed, a random or classic island, the turn timer (off, or 30 seconds to 3 minutes), the points needed to win (5 to 15) and the friendly robber, which keeps the robber away from players with 2 points or fewer. The same seed and player count always build the same island and harbors. The victory screen shows the seed with each player's rolls, resources collected, trades and steals, and the host can deal a new island to the same table with **Play again**.
